classdef TensorUtils
    ...
    methods(Static) % Mapping, construction via callback
        function varargout = mapToSizeFromSubs(sz, varargin)
            ...
            if isempty(sz)
                varargout = ERAASR.TensorUtils.cellvec(nargout);
                for i = 1:nargout
                    if asCell
                        varargout{i} = {};
                    else
            ...
            sz = ERAASR.TensorUtils.expandSizeToNDims(sz, p.Results.minDims);
            nDims = length(sz);
...
%             end
        ...
        function results = mapIncludeSubs(fn, varargin)
            ...
            tSubs = ERAASR.TensorUtils.containingSubscripts(size(varargin{1}));
        ...
        function varargout = mapIncludeSubsAndSize(fn, varargin)
            ...
            [varargout{1:nargout}] = ERAASR.TensorUtils.mapIncludeSubs(fnWrap, varargin{:});
        end
        ...
        function varargout = mapSlices(fn, spanDim, varargin)
            ...
            tCellArgs = cellfun(@(t) ERAASR.TensorUtils.selectEachAlongDimension(t, dim), ...
            ...
            % varargout = cellfun(@(r) ERAASR.TensorUtils.reassemble(r, dim), resultCell, 'UniformOutput', false);
        end
        ...
        function varargout = mapSlicesInPlace(fn, spanDim, varargin)
            ...
            [resultCell{1:nargout}] = ERAASR.TensorUtils.mapSlices(fn, spanDim, varargin{:});
            nonSpanDims = ERAASR.TensorUtils.otherDims(size(resultCell{1}), spanDim, ndims(varargin{1}));
            varargout = cellfun(@(r) ERAASR.TensorUtils.reassemble(r, nonSpanDims, ndims(varargin{1})), ...
        ...
        function varargout = mapFromAxisLists(fn, axisLists, varargin)
            ...
            [varargout{1:nargout}] = ERAASR.TensorUtils.mapToSizeFromSubs(sz, @indexFn, 'asCell', p.Results.asCell);
            
            function varargout = indexFn(varargin)
                inputs = ERAASR.TensorUtils.cellvec(numel(axisLists));
        ...
        function t = buildCombinatorialStructTensor(varargin)
            ...
            for iArg = 1:nArg
                ...
                thisAsCellOriented = ERAASR.TensorUtils.orientSliceAlongDims(thisAsCell, [iArg, nArg+1]);
                % and expand to be the same size as the full combinatorial tensor
                asCells{iArg} = ERAASR.TensorUtils.singletonExpandToSize(thisAsCellOriented, nByArg);
            end
        ...
        function S = buildCombinatorialStringTensorFromLists(listsByDim, joinWith)
            ...
            S = ERAASR.TensorUtils.mapFromAxisLists(@horzcat, listsByDim);
        end
        ...
        function t = mapCatToTensor(fn, varargin)
            ...
            % tensor
            t = ERAASR.TensorUtils.map(fn, varargin{:});
            if iscell(t)
                t = cell2mat(t);
            end
        end
    ...
    methods(Static) % Dimensions and sizes
        ...
        function sz = sizeMultiDim(t, dims)
            ...
            szAll = ERAASR.TensorUtils.expandSizeToNDims(size(t), max(dims));
            sz = arrayfun(@(d) szAll(d), dims);
        end
        
        function sz = sizeOtherDims(t, excludeDims)
            % sz = sizeOtherDims(t, excludeDims)
            otherDims = ERAASR.TensorUtils.otherDims(size(t), excludeDims);
            sz = ERAASR.TensorUtils.sizeMultiDim(t, otherDims);
        end
        ...
        function sz = sizeNDims(t, nDims)
            sz = ERAASR.TensorUtils.expandSizeToNDims(size(t), nDims);
        end
        ...
        function tf = compareSizeVectors(sz1, sz2)
            ...
            tf = isequal(ERAASR.TensorUtils.expandSizeToNDims(sz1, nDims), ....
                ERAASR.TensorUtils.expandSizeToNDims(sz2, nDims));
        end
        ...
        function other = otherDims(sz, dims, ndims)
            ...
            allDims = 1:ndims;
            other = ERAASR.TensorUtils.makerow(setdiff(allDims, dims));
        end
    ...
    methods(Static) % Mask generation and mask utilities
       function idx = vectorMaskToIndices(mask)
            if islogical(mask)
                idx = ERAASR.TensorUtils.makecol(find(mask));
            else
                idx = ERAASR.TensorUtils.makecol(mask);
            end
        end
        
        function mask = vectorIndicesToMask(idx, len)
            if islogical(idx)
                mask = ERAASR.TensorUtils.makecol(idx);
            else
                if ~isscalar(len) && isvector(len), len = numel(len); end % assume its a mask of the same size
                mask = ERAASR.TensorUtils.falsevec(len);
                mask(idx) = true;
            end
        end
        ...
        function maskNew = subselectVectorMask(maskOrig, maskAnd)
            ...
            keep = ERAASR.TensorUtils.vectorIndicesToMask(existingKeep(maskAnd), numel(maskOrig));
            maskNew = maskOrig;
            maskNew(~keep) = false;
        end
        ...
        function inflated = inflateMaskedTensor(maskedTensor, dims, masks, fillWith)
            ...
            for i = 1:numel(masks)
                if ~islogical(masks{i})
                    masks{i} = ERAASR.TensorUtils.vectorIndicesToMask(masks{i}, max(masks{i}));
                end
            end
            ...
            masks = ERAASR.TensorUtils.makecol(masks);
                
            nInflatedVec = cellfun(@numel, masks);
            nSelectedVec = cellfun(@nnz, masks);
            
            % check size along selected dims
            assert(isequal(ERAASR.TensorUtils.makecol(ERAASR.TensorUtils.sizeMultiDim(maskedTensor, dims)), nSelectedVec), ...
            ...
            maskByDim = ERAASR.TensorUtils.maskByDimCellSelectAlongDimension(inflatedSize, dims, masks);
    ...
    methods(Static) % Indices and subscripts
        ...
        function t = containingLinearInds(sz)
            ...
            sz = ERAASR.TensorUtils.expandScalarSize(sz);
            t = ERAASR.TensorUtils.mapToSizeFromSubs(sz, @(varargin) sub2ind(sz, varargin{:}), false);
        end
        ...
        function t = containingSubscripts(sz, asCell)
            ...
            if asCell
                t = ERAASR.TensorUtils.mapToSizeFromSubs(sz, @(varargin) varargin, true);
            else
                t = ERAASR.TensorUtils.mapToSizeFromSubs(sz, @(varargin) [varargin{:}]', true);
            end
        end
        ...
        function t = containingSubscriptsCatAlongFirstDimension(sz)
            ...
            t = cell2mat(shiftdim(ERAASR.TensorUtils.containingSubscripts(sz), -1));
        end
        ...
        function mat = ind2subAsMat(sz, inds)
            ...
            [subsCell{:}] = ind2sub(sz, ERAASR.TensorUtils.makecol(inds));
    ...
    methods(Static) % Selection Mask generation
        ...
        function maskByDim = maskByDimCellSelectAlongDimension(sz, dim, select)
            ...
            maskByDim = ERAASR.TensorUtils.maskByDimCell(sz);
        ...
        function mask = maskSelectAlongDimension(sz, dim, select)
            ...
            mask = false(sz);
            maskByDim = ERAASR.TensorUtils.maskByDimCellSelectAlongDimension(sz, dim, select);
            mask(maskByDim{:}) = true;
        end
        ...
        function mask = maskSelectPartialFromOrigin(sz, dim, select)
            ...
            mask = ERAASR.TensorUtils.maskSelectAlongDimension(sz, dim, selectCell);
        end
    end
    ...
    methods(Static) % Squeezing along particular dimensions
        ...
        function tsq = squeezeOtherDims(t, dims)
            other = ERAASR.TensorUtils.otherDims(size(t), dims);
            tsq = ERAASR.TensorUtils.squeezeDims(t, other);
        end
    end
    ...
    methods(Static) % Regrouping, Nesting, Selecting, Reshaping
        function tCell = regroupAlongDimension(t, dims)
            ...
            tCell = ERAASR.TensorUtils.squeezeSelectEachAlongDimension(t, dims);
            tCell = ERAASR.TensorUtils.squeezeOtherDims(tCell, dims);
        end
        
        function tCell = nestedRegroupAlongDimension(t, dimSets)
            assert(iscell(dimSets), 'dimSets must be a cell array of dimension sets');
            
            dimSets = ERAASR.TensorUtils.makecol(cellfun(@ERAASR.TensorUtils.makecol, dimSets, 'UniformOutput', false));
            allDims = cell2mat(dimSets);
            
            assert(length(unique(allDims)) == length(allDims), ...
                'A dimension was included in multiple dimension sets');
            
            otherDims = ERAASR.TensorUtils.otherDims(size(t), allDims);
            ...
            function tCell = inner(t, dimSets)
                ...
                elseif length(dimSets) == 2
                    ...
                    tCell = ERAASR.TensorUtils.regroupAlongDimension(t, dimSets{1});
                else
                    % call inner on each slice of dimSets{1}
                    remainingDims = ERAASR.TensorUtils.otherDims(size(t), dimSets{1});
                    tCell = ERAASR.TensorUtils.mapSlices(@(t) mapFn(t, dimSets), ...
                        remainingDims, t);
                    tCell = ERAASR.TensorUtils.squeezeOtherDims(tCell, dimSets{1});
                end
            end
            
            function tCell = mapFn(t, dimSets)
                remainingDimSets = cellfun(...
                    @(dims) ERAASR.TensorUtils.shiftDimsPostSqueeze(size(t), dimSets{1}, dims), ...
                    dimSets(2:end), 'UniformOutput', false);
                tCell = inner(ERAASR.TensorUtils.squeezeDims(t, dimSets{1}), remainingDimSets);
            end
        ...
        function [res, maskByDim] = selectAlongDimension(t, dim, select, squeezeResult)
            ...
            if squeezeResult
                ...
                res = ERAASR.TensorUtils.squeezeDims(res, dim);
            end
        end
        
        function t = assignIntoTensorAlongDimension(t, assignThis, dims, select)
            sz = size(t);
            maskByDim = ERAASR.TensorUtils.maskByDimCellSelectAlongDimension(sz, dims, select);
        ...
        function selected = selectMaskAlongMultipleDimensions(t, dims, mask)
            ...
            sz = size(t);
            otherDims = ERAASR.TensorUtils.otherDims(sz, dims);
            t_reshape = ERAASR.TensorUtils.reshapeByConcatenatingDims(t, {otherDims, dims});
        ...
        function t = assignIntoTensorAlongMultipleDimensionsByMask(t, assignThis, dims, mask)
            ...
            if isscalar(assignThis)
                ...
                assignThis = ERAASR.TensorUtils.reshapeByConcatenatingDims(assignThis, {otherDims, dims});
                t_reshape(:, mask) = assignThis;
            end
            
            t = ERAASR.TensorUtils.undoReshapeByConcatenatingDims(t_reshape, {otherDims, dims}, sz);
        end
        ...
        function sel = selectAlongDimensionWithNaNs(t, dim, select, varargin)
            ...
            selNoNan = ERAASR.TensorUtils.selectAlongDimension(t, dim, select(~nanMask), varargin{:});
            sel = ERAASR.TensorUtils.inflateMaskedTensor(selNoNan, dim, ~nanMask);
        end
        ...
        function [res, maskByDim] = squeezeSelectAlongDimension(t, dim, select)
            ...
            [res, maskByDim] = ERAASR.TensorUtils.selectAlongDimension(t, dim, select, true);
        end
        ...
        function tCell = selectEachAlongDimension(t, dim, squeezeEach)
            ...
            %maskByDimCell = ERAASR.TensorUtils.maskByDimCell(sz);
            ...
            % oh so clever
            tCell = ERAASR.TensorUtils.mapToSizeFromSubs(szResult, 'minDims', max(dim), 'asCell', true, ...
                'contentsFn', @(varargin) ERAASR.TensorUtils.selectAlongDimension(t, dim, ...
        ...
        function tCell = squeezeSelectEachAlongDimension(t, dim)
            ...
            tCell = ERAASR.TensorUtils.selectEachAlongDimension(t, dim, true);
        end
        ...
        function t = reassemble(tCell, dim, nd)
            ...
            %subs = ERAASR.TensorUtils.containingSubscripts(szT);
            t = ERAASR.TensorUtils.mapToSizeFromSubs(szT, @getElementT, true);
        ...
        function vec = flatten(t)
            vec = ERAASR.TensorUtils.makecol(t(:));
        end
        ...
        function mat = flattenAlongDimension(t, dim)
            ...
            sqMask = ERAASR.TensorUtils.maskByDimCell(size(t));
            for iAlong = 1:nAlong
                sqMask{dim} = iAlong;
                within = t(sqMask{:});
                mat(iAlong, :) = within(:);
        ...
        function tCell = flattenAlongDimensionAsCell(t, dim)
            ...
            % values from each t(..., i, ...) where i is along dim
            tCell = ERAASR.TensorUtils.regroupAlongDimension(t, dim);
        ...
        function [out, which] = catWhich(dim, varargin)
            ...
            if nargout > 1
                which = cell2mat(ERAASR.TensorUtils.makecol(cellfun(@(in, idx) idx*ones(size(in, dim), 1), varargin, ...
        ...
        function paddedCell = expandToSameSizeAlongDims(dims, varargin)
            ...
            szMat = cell2mat(cellfun(@(x) ERAASR.TensorUtils.sizeNDims(x, nd), ERAASR.TensorUtils.makecol(varargin), 'UniformOutput', false));
            
            szPadTo = max(szMat, [], 1);
            padFn = @(x) ERAASR.TensorUtils.expandOrTruncateToSize(x, dims, szPadTo(dims));
            
            paddedCell = cellfun(padFn, ERAASR.TensorUtils.makecol(varargin), 'UniformOutput', false);
        end
        ...
        function out = catPad(dim, varargin)
            ...
            paddedCell = ERAASR.TensorUtils.expandToSameSizeAlongDims(otherDims, varargin{:});
        ...
        function [out, tvec] = concatenateAlignedToCommonTimeVector(timeCell, dataCell, timeDim, catDim, varargin)
            ...
            szMat = cell2mat(cellfun(@(x) ERAASR.TensorUtils.sizeNDims(x, nd), dataCell, 'UniformOutput', false));
            ...
            for i = 1:Ncat
                ti = timeCell{i};
                idxStart = ERAASR.TensorUtils.argMin(abs(ti(1) - tvec));
                idxEnd = idxStart + numel(ti) - 1;
                
                dims = [catDim, timeDim];
                masks = {i, idxStart:idxEnd};
                
                out = ERAASR.TensorUtils.assignIntoTensorAlongDimension(out, dataCell{i}, dims, masks);
            end                 
        end
        ...
        function dataNew = sliceOrExpandToAlignTimeVector(tvecCurrent, dataCurrent, tvecNew, timeDim)
            ...
            dataSlice = ERAASR.TensorUtils.selectAlongDimension(dataCurrent, timeDim, mask);
            tvecCurrent = tvecCurrent(mask);
            
            % then expand as neede
            sz = ERAASR.TensorUtils.sizeNDims(dataSlice, timeDim);
            sz(timeDim) = numel(tvecNew);
            dataNew = nan(sz, 'like', dataSlice);
            
            idxStart = ERAASR.TensorUtils.argMin(abs(min(tvecCurrent) - tvecNew));
            idxEnd = idxStart + numel(tvecCurrent) - 1;
            dataNew = ERAASR.TensorUtils.assignIntoTensorAlongDimension(dataNew, dataSlice, timeDim, idxStart:idxEnd);
        end
        ...
        function [out, which] = catWhichIgnoreEmpty(dim, varargin)
            ...
            if nargout > 1
                whichMasked = cell2mat(ERAASR.TensorUtils.makecol(cellfun(@(in, idx) idx*ones(size(in, dim), 1), varargin(~isEmpty), ...
                ...
                which = ERAASR.TensorUtils.indicesIntoMaskToOriginalIndices(whichMasked, ~isEmpty);
            end
        end
        ...
        function [out, which] = catInnerDimOverOuterDim(t, innerDim, outerDim)
            ...
            if nargout == 2
                [out, whichCell]  = ERAASR.TensorUtils.mapSlices(@(slice) ERAASR.TensorUtils.catWhich(innerDim, slice{:}), outerDim, t);
                which = whichCell{1};
            else
                out = ERAASR.TensorUtils.mapSlices(@(slice) cat(innerDim, slice{:}), outerDim, t);
            end
        end
        ...
        function [out, labelsByDimOut] = reshapeByConcatenatingDims(in, whichDims, labelsByDim)
            ...
            whichDims = ERAASR.TensorUtils.makecol(whichDims);
            
            allDims = cellfun(@(x) x(:), whichDims, 'UniformOutput', false);
            allDims = cat(1, allDims{:});
            
            szIn = ERAASR.TensorUtils.sizeNDims(in, max(allDims));
            ...
            if nargout > 1
                szInExpand = ERAASR.TensorUtils.expandScalarSize(szIn);
                ...
                szOut = size(out);
                labelsByDimOut = ERAASR.TensorUtils.cellvec(ndimsOut);
                for iDimOut = 1:ndimsOut
                    dimsFromIn = whichDims{iDimOut};
                    % temporary storage of the columns of
                    % labelsByDimOut{iDimOut}, before concatenation
                    labelsThisOut = ERAASR.TensorUtils.cellvec(numel(dimsFromIn));
                    subsCell = ERAASR.TensorUtils.cellvec(numel(dimsFromIn));
                    ...
                    for iDimFromIn = 1:numel(dimsFromIn)
                        ...
                        if isvector(labelsThisIn) && length(labelsThisIn) == size(in, dimsFromIn(iDimFromIn))
                            labelsThisIn = ERAASR.TensorUtils.makecol(labelsThisIn);
                        end
        ...
        function [out, newDims] = reshapeDimsInPlace(in, whichDims, newSizeInThoseDims)
            ...
            sz = size(in);
            assert(isequal(ERAASR.TensorUtils.makecol(whichDims), ERAASR.TensorUtils.makecol(min(whichDims):max(whichDims))), 'Dims must be consecutive');
            
            nElements = prod(ERAASR.TensorUtils.sizeMultiDim(in, whichDims));
        ...
        function out = flattenDimsInPlace(in, whichDims)
            newLen = prod(ERAASR.TensorUtils.sizeMultiDim(in, whichDims));
            out = ERAASR.TensorUtils.reshapeDimsInPlace(in, whichDims, newLen);
        end
        ...
        function out = selectSetsAlongDimension(in, dim, selectIdxCell)
            ...
            otherDims = ERAASR.TensorUtils.otherDims(size(in), dim);
            each = cellfun(@(idx) ERAASR.TensorUtils.squeezeSelectEachAlongDimension(...
                ERAASR.TensorUtils.selectAlongDimension(in, dim, idx), otherDims), selectIdxCell, ...
                'UniformOutput', false);
            out = cat(dim, each{:});
        end
        ...
        function out = selectSpecificIndicesAlongDimensionEachPosition(in, dim, idxThatDim)
            ...
            out = ERAASR.TensorUtils.mapSlices(@selectIdx, dim, in, idxThatDim);
            if ~iscell(in)
                out = cell2mat(out);
                out(~maskValid) = NaN;
            else
    ...
    methods(Static) % Slice orienting and repmat
        ...
        function out = orientSliceAlongDims(slice, spanDim)
            ...
            if ndimsSlice == 1
                % when spanDim is scalar, expect column vector or ERAASR.TensorUtils.makecol
                slice = ERAASR.TensorUtils.makecol(squeeze(slice));
            end
        ...
        function out = orientSliceAlongSameDimsAs(slice, refSlice)
            ...
            if ~isempty(spanDim);
                out = ERAASR.TensorUtils.orientSliceAlongDims(slice, spanDim);
            else
                out = slice;
            end
        end
        ...
        function out = repmatSliceAlongDims(slice, szOut, spanDim)
            ...
            sliceOrient = ERAASR.TensorUtils.orientSliceAlongDims(slice, spanDim);
        ...
        function t = unshiftdimToFirstDim(t, dim, ndimsOrig)
            t = ERAASR.TensorUtils.unshiftdim(t, dim-1, ndimsOrig);
        end   
    end
    ...
    methods(Static) % Size expansion / truncation
        function out = scalarExpandToSize(in, szOut)
            if isscalar(in)
                ...
                szIn = size(in);
                szOut = ERAASR.TensorUtils.expandScalarSize(szOut);
        ...
        function out = expandAlongDims(in, dims, by, fillWith)
            ...
            if nargin < 4
                out = ERAASR.TensorUtils.emptyWithSameType(in, sz);
            else
                out = repmat(fillWith, sz);
            end
            
            % build the mask over out to assign in into
            szInDims = ERAASR.TensorUtils.sizeMultiDim(in, dims);
            maskByDim = ERAASR.TensorUtils.maskByDimCellSelectPartialFromOrigin(size(out), dims, szInDims);
        ...
        function t = expandOrTruncateToSize(t, dims, makeSize)
            ...
            sz = ERAASR.TensorUtils.sizeMultiDim(t, dims);
            makeSize = ERAASR.TensorUtils.makerow(makeSize);
            expandBy = makeSize - sz;
                
            tooSmall = expandBy > 0;
            if any(tooSmall)
                t = ERAASR.TensorUtils.expandAlongDims(t, dims(tooSmall), expandBy(tooSmall));
            end
            
            tooLarge = expandBy < 0;
            if any(tooLarge)
                maskByDim = ERAASR.TensorUtils.maskByDimCellSelectPartialFromOrigin(size(t), dims(tooLarge), makeSize(tooLarge));
                t = t(maskByDim{:});
            end
        end
    end
    ...
    methods(Static) % List resampling and shuffling along dimension
        ...
        function t = listShuffleAlongDimension(t, iA, replace)
            ...
            t = ERAASR.TensorUtils.mapSlicesInPlace(@shuffleFn, iA, t);
            
            function sNew = shuffleFn(s)
                list = ERAASR.TensorUtils.combineListFromCells(s);
                if ~isscalar(list)
                ...
                sNew = ERAASR.TensorUtils.splitListIntoCells(list, cellfun(@numel, s));
            end
        end
        ...
        function t = listResampleFromSame(t)
            ...
            t = ERAASR.TensorUtils.map(@(list) list(randsample(numel(list), numel(list), true)), t);
        end
        ...
        function t = listResampleFromSpecifiedAlongDimension(t, from, iA, replace)
            ...
            t = ERAASR.TensorUtils.mapSlicesInPlace(@resampleFn, iA, t);
            
            function sNew = resampleFn(s)
                sNew = cell(size(s));
                for i = 1:numel(s)
                    list = ERAASR.TensorUtils.combineListFromCells(s(from{i}));
        ...
        function list = combineListFromCells(t)
            ...
            t = cellfun(@ERAASR.TensorUtils.makecol, t, 'UniformOutput', false);
        ...
        function t = splitListIntoCells(list, nPerCell)
            ...
            if isempty(list)
                ...
                t = ERAASR.TensorUtils.cellvec(numel(nPerCell));
            else
                t = mat2cell(ERAASR.TensorUtils.makecol(list), nPerCell(:), 1);
            end
        ...
        function c = splitAlongDimension(t, dim, nPerCell)
           ...
           args = ERAASR.TensorUtils.cellvec(ndims(t));
           if nargin < 3
               nPerCell = arrayfun(@(dim) ERAASR.TensorUtils.onesvec(size(t, dim)), dim, 'UniformOutput', false);
           end
           if ~iscell(nPerCell)
               nPerCell = {nPerCell};
           end
        ...
        function out = splitAlongDimensionByIndex(t, dim, which)
            ...
            N = max(which);
            out = arrayfun(@(i) ERAASR.TensorUtils.selectAlongDimension(t, dim, which == i), (1:N)', 'UniformOutput', false);
        end
    ...
    methods(Static) % Multi-dim extensions of any, all, find etc.
        ...
        function idxTensor = findNAlongDim(t, dim, N, direction)
            ...
            % and orientation
            sizeSlice = ERAASR.TensorUtils.onesvec(max(dim, ndims(t)))';
            sizeSlice(dim) = N;
            emptySlice = nan(sizeSlice);
            
            idxTensor = cell2mat(ERAASR.TensorUtils.mapSlices(@findInner, dim, t));
    ...
    methods(Static) % Statistics       
        function t = meanMultiDim(t, dims)
            ...
            t = ERAASR.TensorUtils.mapSlicesInPlace(@(slice) mean(slice(:)), dims, t);
        end
        ...
        function t = nanmeanMultiDim(t, dims)
            ...
            % will compute the mean in slices along dims 2 and 3. the
            % result will have size s1 x 1 x 1 x s4
            t = ERAASR.TensorUtils.mapSlicesInPlace(@(slice) nanmean(slice(:)), dims, t);
        end
        ...
        function t = varMultiDim(t, normOpt, dims)
            ...
            % will compute the mean in slices along dims 2 and 3. the
            % result will have size s1 x 1 x 1 x s4
            t = ERAASR.TensorUtils.mapSlicesInPlace(@(slice) var(slice(:), normOpt), dims, t);
        end
        ...
        function t = nanvarMultiDim(t, normOpt, dims)
            ...
            % will compute the mean in slices along dims 2 and 3. the
            % result will have size s1 x 1 x 1 x s4
            t = ERAASR.TensorUtils.mapSlicesInPlace(@(slice) var(slice(:), normOpt, 'omitnan'), dims, t);
        end
        ...
        function t = stdMultiDim(t, dims, varargin)
            ...
            % will compute the mean in slices along dims 2 and 3. the
            % result will have size s1 x 1 x 1 x s4
            t = ERAASR.TensorUtils.mapSlicesInPlace(@(slice) std(slice(:), varargin{:}), dims, t);
        end
        ...
        function [y, group] = buildAnovanInputs(valueCell, namesAlongAxes)
            ...
            group = ERAASR.TensorUtils.cellvec(nAx);
            for a = 1:nAx
                group{a} = cell(sz);
            end
            
            % construct the cell of factor level names
            subs = ERAASR.TensorUtils.cellvec(nAx);
            for i = 1:numel(valueCell)
                ...
                valueCell{i} = ERAASR.TensorUtils.makecol(valueCell{i});
            end
        ...
        function [idxAlongDim, val] = argMin(t, dim)
            ...
            % position
            if nargin < 2
                dim = ERAASR.TensorUtils.firstNonSingletonDim(t);
            end
            [val, idxAlongDim] = ERAASR.TensorUtils.mapSlices(@min, dim, t);
            val = cell2mat(val);
        ...
        function [idxAlongDim, val] = argMax(t, dim)
            % returns the position along dim of the minimum of each
            % position
            [val, idxAlongDim] = ERAASR.TensorUtils.mapSlices(@max, dim, t);
            val = cell2mat(val);
            idxAlongDim = cell2mat(idxAlongDim);
        end
        ...
        function [coeff, score, latent, tsquared, explained] = pcaMultiDim(t, basisDims, varargin)
            ...
            szOrig = size(t);
            otherDims = ERAASR.TensorUtils.otherDims(szOrig, basisDims);
            t = ERAASR.TensorUtils.reshapeByConcatenatingDims(t, {otherDims, basisDims});
            
            % deal gracefully with all nan bases
            validCols = any(~isnan(t), 1);
            [coeff, score, latent, tsquared, explained] = pca(t(:, validCols), varargin{:});
            if ~all(validCols)
                coeff = ERAASR.TensorUtils.inflateMaskedTensor(coeff, 1, validCols);
            end
            nC = size(coeff, 2);
        ...
        function [coeff, score, latent, tsquared, explained, mu] = pcaAlongDim(t, basisDim, varargin)
            ...
            szOrig = size(t);
            otherDims = ERAASR.TensorUtils.otherDims(szOrig, basisDim);
            t = ERAASR.TensorUtils.reshapeByConcatenatingDims(t, {otherDims, basisDim});
            ...
            if ~all(validCols)
                coeff = ERAASR.TensorUtils.inflateMaskedTensor(coeff, 1, validCols, 0);
                mu = ERAASR.TensorUtils.inflateMaskedTensor(mu', 1, validCols, 0);
            end
            
            szNew = szOrig;
            nC = size(coeff, 2);
            szNew(basisDim) = nC;
            score = ERAASR.TensorUtils.undoReshapeByConcatenatingDims(score, {otherDims, basisDim}, szNew); 
            tsquared = reshape(tsquared, szOrig(otherDims));
            
            mu = ERAASR.TensorUtils.orientSliceAlongDims(mu, basisDim);
        end
    end
    ...
    methods(Static) % Data manipulation
        function t = centerSlicesOrthogonalToDimension(t, alongDims)
            ...
            otherDims = ERAASR.TensorUtils.otherDims(size(t), alongDims);
            meanTensor =  ERAASR.TensorUtils.nanmeanMultiDim(t, otherDims);
        ...
        function t = centerSlicesSpanningDimension(t, alongDims)
            ...
            % mean.
            meanTensor =  ERAASR.TensorUtils.nanmeanMultiDim(t, alongDims);
            t = bsxfun(@minus, t, meanTensor);
        end
        ...
        function t = zscoreAlongDimension(t, alongDims)
            ...
            t = ERAASR.TensorUtils.centerSlicesSpanningDimension(t, alongDims);
            stdTensor = ERAASR.TensorUtils.stdMultiDim(t, alongDims);
        ...
        function reweightedTensor = linearCombinationAlongDimension(t, dim, weightsNewByOld, varargin)
            ...
            pdims = [dim, ERAASR.TensorUtils.otherDims(sz, dim)];
            tp = permute(t, pdims);
        ...
        function newTensor = linearCombinationApplyScalarFnAlongDimension(t, dim, logicalNewByOld, fn, varargin)
            ...
            parts = ERAASR.TensorUtils.cellvec(nNew);
            for iNew = 1:nNew
                fnSelect = @(c) fn(c(logicalNewByOld(iNew, :) ~= 0));
                parts{iNew} = ERAASR.TensorUtils.mapSlices(fnSelect, dim, t);
            end
        ...
        function in = assignValueMaskedSelectionAlongDimension(in, dims, mask, value)
            ...
            maskByDim = ERAASR.TensorUtils.maskByDimCellSelectAlongDimension(size(in), dims, mask);
            in(maskByDim{:}) = value;
        end
